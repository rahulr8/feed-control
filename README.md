...
3. Browse Reddit, X or LinkedIn. It works out of the box: no account, no key.

Privacy: see [PRIVACY.md](PRIVACY.md).

## Server (the shared key)
`server/` is a one-function Vercel project (`feed-control-api.vercel.app`). The extension sends every request there; it holds the OpenRouter key so users need none. Defenses, from outermost in:
